...
export class WindowsService {
   ...
   * @returns {Promise<any>}
   */
  public changeSize(windowId: string, width: number, height: number): Promise<any> {
    return new Promise(async (resolve, reject) => {
      try {
        await this._obtainWindow(windowId);
   ...
   * @returns {Promise<any>}
   */
  public changePosition(windowId: string, left: number, top: number): Promise<any> {
    return new Promise(async (resolve, reject) => {
      try {
        await this._obtainWindow(windowId);
  ...
  public async getStartupWindowName(): Promise<string> {
    ...
    if (launchSource === 'gamelaunchevent') {
      return WindowNames.LEADERBOARD;
    }
